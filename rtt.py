...
def main():
    ...
    palabras_elegidas = elegir_palabras(dicDePalabras, frase_aleatoria, listFraseEnDos)
    ...
    while True:
        ...
        for i in range(len(palabras_elegidas)):
            if (numero - 1) == i and palabra == palabras_elegidas[i]:
        ...
        numPos = frase_aleatoria[1]

        imprimir(numero, palabra, palabras_elegidas, numPos, dicDePalabras, numCorrectos)
        ...
        for fila in range(len(palb)):
            if fila < 9:
                print('{}.  {}'.format(fila + 1, palb[fila][:int(numpos[0])] + palb[fila][int(numpos[0])].upper() +
                                       palb[fila][(int(numpos[0]) + 1):int(numpos[1])] + palb[fila][
                                           int(numpos[1])].upper() + palb[fila][int(numpos[1]) + 1:]))
            if fila >= 9:
...
def imprimir(numero, palabra, palb, pos, dicDePalabras, numcorrectos):
    ...
    for fila in range(len(palb)):
        if len(numcorrectos) != 0:
            for i in numcorrectos:
                if i == fila and (numero-1) == i:
                    if fila < 9:
...
def elegir_palabras(dicPalabras, frase_aleatoria, listFrase):
    ...
    numPos1, numPos2 = frase_aleatoria[1]  # lista que contiene las columnas
...
def abrirArchivo(archivo):
    '''recibe un archivo con el formato columna1|columa2|columna3 y devuelve el contenido en un diccionario'''
    diccionario = {}
